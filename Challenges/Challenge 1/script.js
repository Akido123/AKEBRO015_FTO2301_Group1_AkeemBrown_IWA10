// Various South African public holidays are stored as nested sub-objects in a "holidays" object.
// Each holiday is assigned a number from "0 to 8" as a unique key identifier.
// the code should check whether an item has been assigned to the "9" key, if so, it should log the name of the holiday.
// If nothing has been assigned yet, then the following should be logged: "ID 9 not created yet".
// To this end, a copied object is used to store the temporary changes that will be applied.
...
// After applying these changes, you should check whether the "new" date is earlier than the "current" date.
// This will be done by logging "true" to the console if this is the case, alternatively, it should log "false".
// Given that midnight is before 13:25 the following should be logged to the console:
// "New date is earlier: true".
// Only if the date is earlier should it apply the change to "copied".
// After the above changes have been applied to the "temporary object", the user should be "alerted" to what has been changed before the changes are applied to the holidays object.
// If a value has not changed it should log "false" to the console, alternatively, it should log the "new value" to the console.
...
copied = { id: false,name: 'X-mas', date: new Date(`25 December ${currentYear}`) }
...
const randomHoliday = holidays[Math.ceil(Math.random() * 8)]
